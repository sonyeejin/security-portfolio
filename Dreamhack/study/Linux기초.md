...
파일 소유자나 소유 그룹 변경하는 명령어로 root유저만 실행 가능하다.  
...
->chmod 4755 world명령어를 통해 world파일에 실행 권한과 setuid설정을 함.  

추가적으로 setuid만 설정할 경우는 chmod u+s world를 setgid만 설정할 경우는 g+s를 sticky bit는 o+t로 설정한다.  
